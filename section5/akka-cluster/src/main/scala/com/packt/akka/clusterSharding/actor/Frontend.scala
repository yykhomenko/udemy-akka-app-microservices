package com.packt.akka.clusterSharding.actor

import akka.actor.{Actor, ActorLogging, ActorRef}
import akka.cluster.sharding.ClusterSharding
import com.packt.akka.clusterSharding.actor.Counter.CounterMessage
...
class Frontend extends Actor with ActorLogging {
  ...
  def receive = {
    case Tick(Inc) =>
      ...
      counterRegion ! CounterMessage(getId, Counter.Increment)
    case Tick(Dec) =>
      log.info(s"Frontend: Send Decrement message to shard region.")
      counterRegion ! CounterMessage(getId, Counter.Decrement)
    case Tick(Get) =>
      log.info(s"Frontend: Send Get message to shard region.")
      counterRegion ! CounterMessage(getId, Counter.Get)
  }
