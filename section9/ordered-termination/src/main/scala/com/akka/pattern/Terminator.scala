...
object Terminator {
  ...
  def props(childProps: Props, numOfChildren: Int) =
    Props(classOf[Terminator], childProps, numOfChildren)
}
...
class Terminator(childProps: Props, numOfChildren: Int) extends Actor {
  ...
  def killKids(kids: List[ActorRef]): Future[Any] = kids match {

    case Nil => Future(AllDead)
